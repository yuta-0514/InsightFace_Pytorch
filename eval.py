...
def lfw_test(model, img_paths, identity_list, compair_list, batch_size):
    ...
    acc, th = test_performance(features, compair_list)
...
if __name__ == '__main__':
    ...
    weight = torch.load('/mnt/weight/backbone.pth')
    resnet = get_model('r50', dropout=0, fp16=False).cuda()
    resnet.load_state_dict(weight)
    model = torch.nn.DataParallel(resnet)

    identity_list = get_lfw_list('/mnt/lfw/pairs.txt')
    img_paths = [os.path.join('/mnt/lfw/', each) for each in identity_list]

    model.eval()
    lfw_test(model, img_paths, identity_list, '/mnt/lfw/pairs.txt', 64)
